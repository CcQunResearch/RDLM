...
from Main.model import BertWithPEP
from Main.utils import write_log
from Main.collator import PEPDataCollatorForPaddingAndMasking
from Main.dataset import PEPDataset
...
if __name__ == '__main__':
    ...
    pep_epochs = args.pep_epochs
    pep_batch_size = args.pep_batch_size
    ...
    pep_dataset_path = osp.join(dirname, '..', 'Data(PEP)', unsup_dataset, 'dataset')
    nost_vocab_file_path = osp.join(dirname, '..', 'Save', 'Twitter', 'vocab_nost.txt')
    save_path = osp.join(dirname, '..', 'Save', 'Twitter', f'[PEP]{run_name}')
    load_path = osp.join(dirname, '..', 'Save', 'Twitter', load_name)
    log_path = osp.join(dirname, '..', 'Log', f'[PEP]{run_name}.log')
    ...
    dataset = PEPDataset(pep_dataset_path, tokenizer, max_position_embeddings, post_num_limit)
    collator = PEPDataCollatorForPaddingAndMasking(tokenizer=tokenizer, mlm=True, mlm_probability=0.12)
    ...
    model = BertWithPEP(config)
    # model.load_model(load_path)
    device = args.gpu if args.cuda else 'cpu'
    model.to(device)

    grad_accumulation_steps = pep_batch_size // 1
    total_steps = len(dataloader) * pep_epochs // pep_batch_size
    ...
    for epoch in range(pep_epochs):
        ...
        for i, claim in enumerate(dataloader):
            ...
            # pep_loss = root_loss + branch_loss + parent_loss
